...
We compare **member vs casual** proportions on **Weekday** vs **Weekend** using a 100% stacked bar to highlight mix over counts.

```{r creating new collumn, message=FALSE, warning=FALSE}
df_daytype <- ride %>%
  count(day_type, member_casual) %>%
  group_by(day_type) %>%
  mutate(pct = n / sum(n)) %>%
  ungroup()


df_daytype$day_type <- factor(df_daytype$day_type, levels = c("Weekday","Weekend"))

ggplot(df_daytype, aes(x = day_type, y = pct, fill = member_casual)) +
  geom_col(position = "fill") +
  geom_text(aes(label = scales::percent(pct, accuracy = 1)),
            position = position_fill(vjust = 0.5),
            color = "white", size = 4) +        
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Trips by Day Type (100%)", x = NULL, y = "Proportion") +
  theme_minimal()
```


**Takeaway.** Members dominate **weekdays** (commute behavior), while **casual riders** dominate **weekends** (leisure) — a clear split that supports targeted conversion offers.
...
```
```{r overview4, message=FALSE, warning=FALSE}
df_hour <- ride %>%
  count(start_hours, member_casual) %>%
  arrange(start_hours)

ggplot(df_hour, aes(x = start_hours, y = n, linetype = member_casual)) +
  geom_line() +
  labs(title = "Trips by Hour (0–23)", x = "Start Hours", y = "Trips Count",
  linetype="Rider Type") +
  theme_minimal()
```

Takeaway. Annual Members peak during commute hours (around 7–9 and 17–18), while Casual Riders ride more steadily, with a slight increase in the afternoon and weekends.
This pattern confirms members = commute use vs casual = leisure use.

### Duration vs Rider Type
I wrote this to see the median trip duration (minutes) for member vs casual.
I’m using median so outliers (super short or super long rides) don’t mess up the comparison.
```{r durationvsrider, message=FALSE, warning=FALSE}
df_med <- ride %>%
  group_by(member_casual) %>%
  summarise(median_min = median(usage_length, na.rm = TRUE))

df_med$member_casual <- factor(df_med$member_casual, levels = c("member", "casual"))

ggplot(df_med, aes(x = member_casual, y = median_min, fill = member_casual)) +
  geom_col() +
  scale_fill_manual(values = c("member" = "#3FBBC0",
                               "casual" = "#EF6F61")) +
  labs(title = "Median Trip Duration", x = NULL, y = "Minutes", fill = "Rider Type") +
  theme_minimal()
```

This fits the pattern I’m seeing overall: casual = more leisure time, members = commute.
